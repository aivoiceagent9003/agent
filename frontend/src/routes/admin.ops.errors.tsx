  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      ...
      <p className="text-sm text-muted-foreground mt-1">
        Every exception and incident captured from the live pipeline.
      </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {Object.entries(byComponent).map(([name, g]: [string, any]) => (
          <button
            key={name}
            onClick={() => setComponent(component === name ? null : name)}
          ...
          >
            ...
            <div className="mt-2 flex gap-1.5 flex-wrap text-[11px]">
              ...
              {g.warning > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-500">
                  {g.warning} warn
                </span>
              )}
            </div>
      ...
      <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mt-10 mb-3">
        Incidents (clustered error/critical events)
      </h2>
            ...
            {incidents.map((i: any, idx: number) => (
              <tr key={idx} className="hover:bg-muted/30">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border ${SEV_COLOR[i.severity]}`}
                  >
                    {i.severity}
                  </span>
                </td>
            ...
            {incidents.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-muted-foreground">
                  ...
                  <div className="mt-1">No incidents.</div>
                </td>
              </tr>
            )}
          </tbody>
      ...
      <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mt-10 mb-3">
        Recent events{" "}
        {component && (
          <button onClick={() => setComponent(null)} className="ml-2 text-primary normal-case">
            ({component} — clear filter)
          </button>
        )}
      </h2>
      <div className="bg-card border border-border rounded-xl shadow-card divide-y divide-border max-h-[480px] overflow-auto">
        {recent.map((ev: any, idx: number) => (
          <div key={idx} className="flex items-start gap-3 px-4 py-3">
            <span
              className={`mt-0.5 text-[10px] px-1.5 py-0.5 rounded-full border shrink-0 ${SEV_COLOR[ev.severity] || SEV_COLOR.info}`}
            >
