...
function AlertCenter() {
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      ...
      <p className="text-sm text-muted-foreground mt-1">Threshold rules evaluated continuously against live telemetry.</p>
      ...
      {active.length === 0 ? (
        ...
        <div className="space-y-2">
          {active.map((a: any) => (
            <div key={a.id} className={`rounded-xl border p-4 shadow-card flex items-center justify-between ${SEV[a.severity]?.badge || SEV.info.badge}`}>
              <div className="flex items-center gap-3">
                <AlertTriangle className="w-5 h-5" />
                <div>
                  <div className="font-semibold">{a.label}</div>
                  <div className="text-xs opacity-80">{a.message} · threshold {a.threshold}</div>
                </div>
              </div>
      ...
      <div className="mt-10 grid lg:grid-cols-2 gap-6">
        {/* Rules */}
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Rules ({rules.length})</h2>
          <div className="bg-card border border-border rounded-xl shadow-card divide-y divide-border">
            {rules.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2.5">
                  <span className={`w-2 h-2 rounded-full ${r.active ? SEV[r.severity]?.dot : "bg-success"}`} />
                  <div>
                    <div className="text-sm font-medium">{r.label}</div>
                    <div className="text-[11px] text-muted-foreground capitalize">{r.group} · {r.severity}</div>
                  </div>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full border ${r.active ? SEV[r.severity]?.badge : "bg-success/15 text-success border-success/30"}`}>
        ...
        {/* History */}
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">History</h2>
            ...
            {history.map((h: any, i: number) => (
              <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm truncate"><span className="font-medium">{h.label}</span> <span className="text-muted-foreground">{h.event}</span></div>
                  {h.event === "resolved" && <div className="text-[11px] text-muted-foreground">lasted {fmtDuration(h.durationMs)}</div>}
                </div>
                <span className="text-xs text-muted-foreground shrink-0">{new Date(h.ts).toLocaleTimeString()}</span>
              </div>
            ))}
            {history.length === 0 && <div className="px-4 py-6 text-center text-sm text-muted-foreground">No alert history yet.</div>}
          </div>
        </div>
      </div>
    </div>
