...
import {
  useWhatsapp, useSaveWhatsapp,
  useSendableDocs, useUploadSendableDoc, useRenameSendableDoc, useDeleteSendableDoc,
} from "@/lib/whatsapp";
...
export function WhatsAppSettings() {
  ...
  if (isLoading || !data) return <div className="text-sm text-muted-foreground">Loading WhatsApp…</div>;
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-600 dark:text-amber-400">
          WhatsApp sending isn't switched on for the platform yet. Once it's live, your agent will send documents and
          confirmations automatically — no setup needed from you.
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          When it fits the conversation, your agent sends the right document or a confirmation to the customer's
          WhatsApp — automatically. Messages go out with <b>your business name</b> and the contact number below, so
          customers know it's from you.
        </p>

        <label className="block max-w-xs">
          <span className="block text-xs font-medium text-muted-foreground mb-1">Contact number to show customers</span>
          <input className={inp} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+9199…" />
        </label>
        ...
        <button onClick={onSave} disabled={save.isPending}
          ...
          {saved ? <><Check className="w-4 h-4" /> Saved</> : save.isPending ? "Saving…" : "Save"}
        </button>
      </div>
    ...
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          The agent sends the document whose <b>topic</b> matches what the caller asked about — so name it the way
          callers say it. These files are only sent to customers; they're separate from{" "}
          <b>Knowledge</b> (which is what your agent reads to answer questions).
        </p>
      </div>

      <div className="rounded-lg border border-dashed border-border p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <input className={`${inp} flex-1 min-w-48`} value={topic} onChange={(e) => setTopic(e.target.value)}
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-muted-foreground">No documents yet — add one per topic above.</div>
      ) : (
        <div className="space-y-2">
          {docs.map((d) => (
            <div key={d.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-border p-2.5">
              ...
              />
              <span className="text-xs text-muted-foreground truncate max-w-56" title={d.filename}>{d.filename}</span>
              <button onClick={() => remove.mutate(d.id)}
                className="p-2 rounded-lg border border-border hover:bg-muted text-destructive" title="Remove">
